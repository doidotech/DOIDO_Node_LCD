...
def display_logo():
    # Load an image.
    image_path = 'images/RaspiBlitz_Logo.png'
    image = disp.buffer
    W, H = (128,160)
    width = 100
...
def display_ip_and_temperature():
    ...
    font13 = ImageFont.truetype("lato/Lato-Bold.ttf", 13)
...
def draw_bitcoin_price():
    ...
    price_font = ImageFont.truetype("/usr/share/fonts/truetype/lato/Lato-Bold.ttf", 26)
    try:
...
def display_syncing_text(is_syncing):
    text_font = ImageFont.truetype("lato/Lato-Bold.ttf", 20)
    if is_syncing:
...
def is_btc_not_synced():
    sync_result = subprocess.run(['./get_btc_sync.sh'], stdout=subprocess.PIPE)
